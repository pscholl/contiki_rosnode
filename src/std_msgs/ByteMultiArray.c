...
{
  ByteMultiArray_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(ByteMultiArray_t)-sizeof(ByteMultiArray_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(ByteMultiArray_t)-sizeof(ByteMultiArray_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(ByteMultiArray_t)-sizeof(ByteMultiArray_t_packed));
  else          var_ptr = to;

  for (size_t i=roslen(obj->data); i-->0;) {
    buf -= sizeof(int8_t);
    var_ptr -= sizeof(int8_t);
    *((int8_t*) (var_ptr)) = ROS_READ8(buf);
  }
  buf -= sizeof(uint32_t);
  *((uint32_t*) (var_ptr-sizeof(uint32_t))) = roslen(obj->data); // store arrlen
  obj->data = (int8_t*) (var_ptr);
  ...
  if (to!=NULL)
      save_ptr = var_ptr - sizeof(ByteMultiArray_t);

  // copy the header back in place.
  memcpy(save_ptr,obj,sizeof(*obj));

  return (ByteMultiArray_t*) save_ptr;
}
