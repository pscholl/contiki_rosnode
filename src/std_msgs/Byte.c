...
{
  Byte_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(Byte_t)-sizeof(Byte_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(Byte_t)-sizeof(Byte_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(Byte_t)-sizeof(Byte_t_packed));
  else          var_ptr = to;
