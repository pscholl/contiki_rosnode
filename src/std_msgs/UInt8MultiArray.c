...
UInt8MultiArray_deserialize(char *buf, size_t n) {
  return UInt8MultiArray_deserialize_size(buf,NULL,&n);
}

UInt8MultiArray_t*
UInt8MultiArray_deserialize_size(char *buf, char *to, size_t *n)
{
  UInt8MultiArray_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(UInt8MultiArray_t)-sizeof(UInt8MultiArray_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(UInt8MultiArray_t)-sizeof(UInt8MultiArray_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(UInt8MultiArray_t)-sizeof(UInt8MultiArray_t_packed));
  else          var_ptr = to;

  for (size_t i=roslen(obj->data); i-->0;) {
    buf -= sizeof(uint8_t);
    var_ptr -= sizeof(uint8_t);
    *((uint8_t*) (var_ptr)) = ROS_READ8(buf);
  }
  buf -= sizeof(uint32_t);
  *((uint32_t*) (var_ptr-sizeof(uint32_t))) = roslen(obj->data); // store arrlen
  obj->data = (uint8_t*) (var_ptr);
  ...
  if (to!=NULL)
      save_ptr = var_ptr - sizeof(UInt8MultiArray_t);

  // copy the header back in place.
  memcpy(save_ptr,obj,sizeof(*obj));

  return (UInt8MultiArray_t*) save_ptr;
}
...
UInt8MultiArray_serialize(UInt8MultiArray_t *obj, char *buf, size_t n)
{
  ...
  for (size_t i=0; i<tmp; i++) {
    ROS_WRITE8(buf,obj->data[i]); buf+=1;
  }
