...
UInt16MultiArray_deserialize(char *buf, size_t n) {
  return UInt16MultiArray_deserialize_size(buf,NULL,&n);
}

UInt16MultiArray_t*
UInt16MultiArray_deserialize_size(char *buf, char *to, size_t *n)
{
  UInt16MultiArray_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(UInt16MultiArray_t)-sizeof(UInt16MultiArray_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(UInt16MultiArray_t)-sizeof(UInt16MultiArray_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(UInt16MultiArray_t)-sizeof(UInt16MultiArray_t_packed));
  else          var_ptr = to;

  for (size_t i=roslen(obj->data); i-->0;) {
    buf -= sizeof(uint16_t);
    var_ptr -= sizeof(uint16_t);
    *((uint16_t*) (var_ptr)) = ROS_READ16(buf);
  }
  buf -= sizeof(uint32_t);
  *((uint32_t*) (var_ptr-sizeof(uint32_t))) = roslen(obj->data); // store arrlen
  obj->data = (uint16_t*) (var_ptr);
  ...
  if (to!=NULL)
      save_ptr = var_ptr - sizeof(UInt16MultiArray_t);

  // copy the header back in place.
  memcpy(save_ptr,obj,sizeof(*obj));

  return (UInt16MultiArray_t*) save_ptr;
}
...
UInt16MultiArray_serialize(UInt16MultiArray_t *obj, char *buf, size_t n)
{
  ...
  for (size_t i=0; i<tmp; i++) {
    ROS_WRITE16(buf,obj->data[i]); buf+=2;
  }
