...
Int16MultiArray_deserialize(char *buf, size_t n) {
  return Int16MultiArray_deserialize_size(buf,NULL,&n);
}

Int16MultiArray_t*
Int16MultiArray_deserialize_size(char *buf, char *to, size_t *n)
{
  Int16MultiArray_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(Int16MultiArray_t)-sizeof(Int16MultiArray_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(Int16MultiArray_t)-sizeof(Int16MultiArray_t_packed));
  ...
  *((uint32_t*) (var_ptr-sizeof(uint32_t))) = roslen(obj->data); // store arrlen
  ...
  if (to!=NULL)
      save_ptr = var_ptr - sizeof(Int16MultiArray_t);

  // copy the header back in place.
  memcpy(save_ptr,obj,sizeof(*obj));

  return (Int16MultiArray_t*) save_ptr;
}
