...
Int32MultiArray_deserialize(char *buf, size_t n) {
  return Int32MultiArray_deserialize_size(buf,NULL,&n);
}

Int32MultiArray_t*
Int32MultiArray_deserialize_size(char *buf, char *to, size_t *n)
{
  Int32MultiArray_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(Int32MultiArray_t)-sizeof(Int32MultiArray_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(Int32MultiArray_t)-sizeof(Int32MultiArray_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(Int32MultiArray_t)-sizeof(Int32MultiArray_t_packed));
  else          var_ptr = to;

  for (size_t i=roslen(obj->data); i-->0;) {
    buf -= sizeof(int32_t);
    var_ptr -= sizeof(int32_t);
    *((int32_t*) (var_ptr)) = ROS_READ32(buf);
  }
  buf -= sizeof(uint32_t);
  *((uint32_t*) (var_ptr-sizeof(uint32_t))) = roslen(obj->data); // store arrlen
  obj->data = (int32_t*) (var_ptr);
  ...
  if (to!=NULL)
      save_ptr = var_ptr - sizeof(Int32MultiArray_t);

  // copy the header back in place.
  memcpy(save_ptr,obj,sizeof(*obj));

  return (Int32MultiArray_t*) save_ptr;
}
