...
{
  Int8MultiArray_t  obj_header, *obj=&obj_header;
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(Int8MultiArray_t)-sizeof(Int8MultiArray_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(Int8MultiArray_t)-sizeof(Int8MultiArray_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(Int8MultiArray_t)-sizeof(Int8MultiArray_t_packed));
  else          var_ptr = to;
  ...
  *((uint32_t*) (var_ptr-sizeof(uint32_t))) = roslen(obj->data); // store arrlen
  ...
  if (to!=NULL)
      save_ptr = var_ptr - sizeof(Int8MultiArray_t);

  // copy the header back in place.
  memcpy(save_ptr,obj,sizeof(*obj));

  return (Int8MultiArray_t*) save_ptr;
}
