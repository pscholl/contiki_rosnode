...
UInt64_deserialize_size(char *buf, char *to, size_t *n)
{
  ...
  if (*n < (buf-save_ptr) + grow_len + (sizeof(UInt64_t)-sizeof(UInt64_t_packed)))
      return NULL; // check if arrays+strings fit
  *n = (buf-save_ptr) + grow_len + (sizeof(UInt64_t)-sizeof(UInt64_t_packed));

  if (to==NULL) var_ptr = buf + grow_len + (sizeof(UInt64_t)-sizeof(UInt64_t_packed));
  else          var_ptr = to;
